...
class RS_SERPs extends Pepper
{
	...
	function getHTML_SERPPie() 
	{
		...
		$html .= $this->generateFilterList("Pie", $filters, array('Most Common', 'Organic', 'Paid'));
		$timespan = ($this->filter) ? " AND dt > ".(time() - ($this->filter * 60 * 60)) : '';
		
		$tableData['table'] = array('id'=>'','class'=>'');
		$tableData['thead'] = array();
		
		$query = "SELECT `serps_engine`, count(*) as total
					FROM `{$this->Mint->db['tblPrefix']}visit` 
					WHERE
					`search_terms`!='' $filter $timespan
					and `serps_page`>0
					GROUP BY `serps_engine`
					ORDER BY `total` DESC
					LIMIT 0,{$this->Mint->cfg['preferences']['rows']}";
					
		$chart = "http://chart.apis.google.com/chart?cht=p&chs=320x180&chds=0,747";
		$counts = array();
		if ($result = $this->query($query))
		{
			while ($r = mysql_fetch_array($result))
		...
		for($i=0;$i<count($engines);++$i)
		{
			$engine = $engines[$i];
			$count = $counts[$i];
			$perc = intval($count*100/$grand);
			$data[] = $perc;
			$labels[] = "$engine ".intval($perc)."%25";
		}
		$chart .= "&chd=t:".join(",",$data);
		$chart .= "&chl=".join("%7C",$labels);
		$chart .= "&chco=7AA054,E5E5E5";
		
		$html .= "<div style=\"text-align: center;\"><img src=\"{$chart}\"/></div>";
		return $html;
	}	
}