...
def templatepage(request, url, template_dir='template_pages/', extra_context={}, context_processors=None):
    ...
    if clean_url and clean_url[-1] == '/':
        clean_url = clean_url[:-1]


    for prefix in (prefix_list):
        try:
            t = loader.get_template(template_dir + prefix + clean_url + '.html')
            mimetype, encoding = guess_type(clean_url + '.html')
            break
        except TemplateDoesNotExist:
            # raise TemplateDoesNotExist(template_dir + prefix + clean_url + '.html') 
            pass
        try:
