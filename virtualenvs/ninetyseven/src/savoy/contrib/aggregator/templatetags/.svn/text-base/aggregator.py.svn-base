...
class GetLatestByModelsNode(template.Node):
    ...
    def render(self, context):
      try:
        ...
        queryset, content_types = filter_queryset_by_model_list(ContentItem.objects.all().order_by("-timestamp").select_related(), model_string_list)
        if int(self.num) == 1:
...
def get_latest_from_models(parser, token):
    """
    Gets the latest [num] ContentItem objects from the aggregator model, such as for use in a mini-tumblelog.
    If [num] is 1, returns a single ContentItem. Otherwise, returns a QuerySet.
    Example::
...
@register.tag
def get_aggregator_items(parser, token):
    """
    Load aggregator ``ContentItem`` objects into the context.In the simplest mode, the
    most recent N items will be returned.

    ::

        {# Fetch 10 most recent items #}
        {% get_aggregator_items limit 10 as items %}
    ...
    unless you ask for them to be reversed::
        ...
        {% get_aggregator_items limit 10 reversed as items %}

    You can also fetch items between two given dates::

        {% get_aggregator_items between "2007-01-01" and "2007-01-31" as items %}

    Dates can be in any format understood by ``dateutil.parser``, and can of
    course be variables. ContentItems must be limited in some way; you must either pass
    ``limit`` or ``between``.

    Dates can also be the magic strings "now" or "today"::

        {% get_aggregator_items between "2007-01-01" and "today" as items %}

    You can also limit items by type::

        {# Fetch the 10 most recent videos and photos #}
        {% get_aggregator_items oftype video oftype photo limit 10 as items %}
    ...
    You can similarly exclude types using ``excludetype``::
        ...
        {% get_aggregator_items excludetype video limit 10 as items %}
    ...
    for arg in ("oftypes", "excludetypes"):
        if arg in args:
            ...
            for name in args[arg]:
                try:
                    model_list.append(ContentItem.objects.models_by_name[name])
                except KeyError:
                    raise template.TemplateSyntaxError("%r tag: invalid model name: %r" % (tagname, name))
            args[arg] = model_list

    return GetAggregatorContentItemsNode(**args)

class GetAggregatorContentItemsNode(template.Node):
