"""
Views for looking at ContentItem items by date.
...
instead of ``ContentContentItem.objects.all()``.
"""
...
from savoy.contrib.aggregator.feeds import LatestItems


def get_feed_url_for_view(model_string):
  feed_base_url = urlresolvers.reverse('aggregator_feeds', args=('latest-items',))
...
def get_aggregator_content_types(type_set="default"):
  content_types = []
...
def filter_queryset_by_model_list(queryset, model_list):
  ...
  for item in model_list:
    try:
      ...
      pass
  for content_type in get_aggregator_content_types(type_set="all"):
...
def archive(request, queryset=None,
    ...
    Templates: ``aggregator/archive.html`` (default)
    Context:
        ``items``
            Items, earliest first.
        ``date_list``
            List of years that have ContentItems.
        ``all_content_types``
...
def year(request, year, queryset=None,
    template_name="aggregator/year.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None):
    """
    ContentItems for a particular year.
    ...
    Templates: ``aggregator/year.html`` (default)
    Context:
        ``items``
    ...
    year = int(year)
    try:
        first = ContentItem.objects.order_by("timestamp")[0]
    except IndexError:
    ...
    previous = year - 1
    previous_link = urlresolvers.reverse("savoy.contrib.aggregator.views.year", args=[previous])
    if previous < first.timestamp.year:
        previous = previous_link = None
    
    # And the next year
    next = year + 1
    next_link = urlresolvers.reverse("savoy.contrib.aggregator.views.year", args=[next])
    if next > today.year:
        next = next_link = None
...
def month(request, year, month, queryset=None,
    template_name="aggregator/month.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None):
    """
    ContentItems for a particular month.
    ...
    Templates: ``aggregator/month.html`` (default)
    Context:
        ``items``
    ...
    previous_link = urlresolvers.reverse("savoy.contrib.aggregator.views.month", args=previous.strftime("%Y %b").lower().split())
    if previous < first.timestamp.date().replace(day=1):
        previous = None
    
    # And the next month
    next = last_day + datetime.timedelta(days=1)
    next_link = urlresolvers.reverse("savoy.contrib.aggregator.views.month", args=next.strftime("%Y %b").lower().split())
    if next > today:
        next = None
...
def day(request, year, month, day, queryset=None, recent_first=True,
    template_name="aggregator/day.html", template_loader=loader,
    extra_context=None, context_processors=None, mimetype=None):
    """
    ContentItems for a particular day.
    ...
    Templates: ``aggregator/day.html`` (default)
    Context:
        ``items``
    ...
    previous_link = urlresolvers.reverse("savoy.contrib.aggregator.views.day", args=previous.strftime("%Y %b %d").lower().split())
    if previous < first.timestamp.date():
        previous = previous_link = None
    
    # And the next month
    next = day + datetime.timedelta(days=1)
    next_link = urlresolvers.reverse("savoy.contrib.aggregator.views.day", args=next.strftime("%Y %b %d").lower().split())
    if next > today:
        next = next_link = None
...
def search(request, template_name="aggregator/search.html", template_loader=loader, extra_context=None, context_processors=None, mimetype=None):
  """
  ContentItem search.
